...
export default function HomeClient({ activities }: HomeClientProps) {
  const hero = useTranslations('hero');
  const about = useTranslations('about');
  const act = useTranslations('activities');
  const sch = useTranslations('schools');
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative min-h-[93vh] bg-white dark:bg-gray-900 overflow-hidden flex items-center justify-center">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10 -mt-16 sm:-mt-20">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
            ...
            >
              ...
              >
                <span className="text-brand-red dark:text-red-400">{hero('titleLine1Start')}</span>{hero('titleLine1End')}
                <br />
                <span className="text-brand-blue dark:text-blue-400">{hero('titleLine2Start')}</span>{hero('titleLine2End')}
              </motion.h1>
              ...
              >
                <p className="text-base sm:text-lg md:text-xl text-gray-700 dark:text-gray-300 mb-6 sm:mb-8 border-l-4 border-brand-gold pl-4 text-center md:text-left mx-auto md:mx-0 max-w-lg md:max-w-none">
                  {hero('description')}
                  <span className="text-brand-red font-semibold">{hero('highlight1')}</span>
                  {hero('middle')}
                  <span className="text-brand-blue font-semibold">{hero('highlight2')}</span>
                  {hero('descEnd')}
                </p>
              </motion.div>
        ...
        >
          <p className="text-gray-500 dark:text-gray-400 mb-2 text-sm">{hero('scrollDown')}</p>
          <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 1.5, repeat: Infinity }}>
            <svg className="w-6 h-6 text-brand-red dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <section id="about" className="py-20 bg-gray-50 dark:bg-gray-800 transition-colors duration-300">
        <div className="container-custom">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
            <h2 className="section-title dark:text-white">{about('title')}<span className="text-brand-red dark:text-red-400">{about('titleHighlight')}</span></h2>
            <div className="max-w-4xl mx-auto">
              <p className="text-lg text-center mb-8 text-gray-700 dark:text-gray-300">{about('intro')}</p>
                ...
                <div className="bg-white dark:bg-gray-700 rounded-xl shadow-lg p-6 md:p-8 transition-all duration-300 hover:shadow-xl">
                  <div className="flex flex-col h-full">
                    <div className="flex items-center mb-4">
                      ...
                      <h3 className="text-2xl font-bold text-brand-red dark:text-red-400">{about('libraryTitle')}</h3>
                    </div>
                    <p className="text-gray-700 dark:text-gray-300 mb-6 flex-grow">{about('libraryDesc')}</p>
                    <div className="flex flex-wrap gap-4 mt-auto">
                      <Link href="/activities-library" className="inline-flex items-center px-4 py-2 bg-brand-red text-white rounded-md hover:bg-opacity-90 transition-all">
                      ...
                      <a href="https://docs.qq.com/form/page/DQk1MaEpLQm9HbnRU#/fill" target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-800 border border-brand-gold text-brand-gold dark:text-yellow-400 rounded-md hover:bg-brand-gold/5 transition-all">
                ...
                <div className="bg-white dark:bg-gray-700 rounded-xl shadow-lg p-6 md:p-8 transition-all duration-300 hover:shadow-xl">
                  <div className="flex flex-col h-full">
                    <div className="flex items-center mb-4">
                      ...
                      <h3 className="text-2xl font-bold text-brand-blue dark:text-blue-400">{about('followTitle')}</h3>
                    </div>
                    <p className="text-gray-700 dark:text-gray-300 mb-6 flex-grow">{about('followDesc')}</p>
                    <div className="flex flex-wrap justify-center gap-6 mt-auto">
                      <a href="https://space.bilibili.com/188894280" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group">
                        <div className="w-14 h-14 bg-white dark:bg-gray-800 flex items-center justify-center rounded-full shadow-md group-hover:shadow-lg transition-all duration-300 mb-2">
                          <svg className="w-8 h-8 text-[#fb7299] group-hover:scale-110 transition-transform duration-300" viewBox="0 0 24 24" fill="currentColor"><path d="M17.813 4.653h.854c1.51.054 2.769.578 3.773 1.574 1.004.995 1.524 2.249 1.56 3.76v7.36c-.036 1.51-.556 2.769-1.56 3.773s-2.262 1.524-3.773 1.56H5.333c-1.51-.036-2.769-.556-3.773-1.56S.036 18.858 0 17.347v-7.36c.036-1.511.556-2.765 1.56-3.76 1.004-.996 2.262-1.52 3.773-1.574h.774l-1.174-1.12a1.234 1.234 0 0 1-.373-.906c0-.356.124-.658.373-.907l.027-.027c.267-.249.573-.373.92-.373.347 0 .653.124.92.373L9.653 4.44c.071.071.134.142.187.213h4.267a.836.836 0 0 1 .16-.213l2.853-2.747c.267-.249.573-.373.92-.373.347 0 .662.151.929.4.267.249.391.551.391.907 0 .355-.124.657-.373.906zM5.333 7.24c-.746.018-1.373.276-1.88.773-.506.498-.769 1.13-.786 1.894v7.52c.017.764.28 1.395.786 1.893.507.498 1.134.756 1.88.773h13.334c.746-.017 1.373-.275 1.88-.773.506-.498.769-1.129.786-1.893v-7.52c-.017-.765-.28-1.396-.786-1.894-.507-.497-1.134-.755-1.88-.773zM8 11.107c.373 0 .684.124.933.373.25.249.383.569.4.96v1.173c-.017.391-.15.711-.4.96-.249.25-.56.374-.933.374s-.684-.125-.933-.374c-.25-.249-.383-.569-.4-.96V12.44c0-.373.129-.689.386-.947.258-.257.574-.386.947-.386zm8 0c.373 0 .684.124.933.373.25.249.383.569.4.96v1.173c-.017.391-.15.711-.4.96-.249.25-.56.374-.933.374s-.684-.125-.933-.374c-.25-.249-.383-.569-.4-.96V12.44c.017-.391.15-.711.4-.96.249-.249.56-.373.933-.373z" /></svg>
                        </div>
                        <span className="text-sm text-gray-600 dark:text-gray-400">{about('bilibili')}</span>
                      </a>
                      <a href="https://github.com/F1Justin/SCTA-Homepage" target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group">
                        <div className="w-14 h-14 bg-white dark:bg-gray-800 flex items-center justify-center rounded-full shadow-md group-hover:shadow-lg transition-all duration-300 mb-2">
                          <svg className="w-8 h-8 text-gray-800 dark:text-white group-hover:scale-110 transition-transform duration-300" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.237 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" /></svg>
                        </div>
                        <span className="text-sm text-gray-600 dark:text-gray-400">{about('github')}</span>
                      </a>
                    </div>
                  </div>
                </div>
      ...
      <section id="activities" className="py-20 bg-white dark:bg-gray-900 relative">
        <div className="container-custom">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
            <h2 className="section-title dark:text-white mb-12">{act('sectionTitle')}<span className="text-brand-red dark:text-red-400">{act('sectionHighlight')}</span></h2>
            <div className="relative">
              <Masonry breakpointCols={masonryBreakpointsHomepage} className="flex w-auto -mx-2 sm:-mx-4" columnClassName="px-2 sm:px-4">
              ...
              <div className="absolute bottom-48 left-0 right-0 flex justify-center">
                <Link href="/activities" className="btn-primary z-10">{act('viewMore')}</Link>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section id="schools" className="py-8 -mt-48 bg-gray-50 dark:bg-gray-800 transition-colors duration-300 relative z-20">
        <div className="container-custom">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="pt-8">
            <h2 className="section-title dark:text-white">{sch('title')}<span className="text-brand-blue dark:text-blue-400">{sch('titleHighlight')}</span></h2>
      ...
      <section id="contact" className="py-20 bg-gradient-to-r from-brand-blue to-brand-red text-white">
        <div className="container-custom">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="max-w-3xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-bold mb-6">{contact('title')}</h2>
            <p className="text-lg mb-8 opacity-90">{contact('desc')}</p>
            <div className="bg-white bg-opacity-10 p-8 rounded-lg backdrop-blur-sm">
              <div className="flex flex-col items-center">
                <div className="bg-white p-4 rounded-lg mb-4">
                  <Image src="/scta-qrcode.png" alt="QR Code" width={192} height={192} className="object-contain" />
                </div>
                <p className="text-lg opacity-90">{contact('scanQr')}</p>
              </div>
            </div>
          </motion.div>
        </div>
